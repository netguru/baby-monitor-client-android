...
public class VisualizerView extends FrameLayout {
    ...
    private Queue<Integer> mVolumeQueue;
    private int mBaseY;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mVolumeQueue = new ArrayDeque(mNumColumns);
        args.recycle();
    }
    ...
    public void receive(final byte[] data, int bufferSize) {
        int volume = calculateDecibel(data, bufferSize);
        if (mVolumeQueue.size() >= mNumColumns) {
            mVolumeQueue.poll();
        }
    ...
    private int calculateDecibel(byte[] buf, int bufferSize) {
        int sum = 0;
    ...
    private void drawBar() {
        for (int i = 0; i < mNumColumns; i++) {
            int volume;
            if (mVolumeQueue.size() > i && mVolumeQueue.toArray()[i] != null) {
                volume = (int) mVolumeQueue.toArray()[i];
            } else {
                volume = 0;
            }
    ...
    private void drawPixel() {
        for (int i = 0; i < mNumColumns; i++) {
            int volume;
            if (mVolumeQueue.toArray()[i] != null) {
                volume = (int) mVolumeQueue.toArray()[i];
            } else {
                volume = 0;
            }
